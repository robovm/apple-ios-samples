    ...
    dispatch_async(dispatch_get_global_queue(0, DISPATCH_QUEUE_PRIORITY_DEFAULT), ^{
        ...
        [self.cloudManager uploadAssetWithURL:localURL completionHandler:^(CKRecord *record) {
            ...
            if (!record) {
                ...
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"CloudKitAtlas" message:@"Successfully Uploaded" preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)downloadPhoto {
    if (!self.assetRecordName) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"CloudKitAtlas" message:@"Upload an asset to retrieve it." preferredStyle:UIAlertControllerStyleAlert];
