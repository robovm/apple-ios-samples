...
- (instancetype) init
{
    ...
    if(self)
    {
        ...
        dispatch_once(&token, ^{
            // Create a dispatch queue for initializing data
            m_DQueue = dispatch_queue_create("com.apple.matrixmult.matrixranddims.main", DISPATCH_QUEUE_CONCURRENT);
...
- (instancetype) init
{
    ...
    if(self)
    {
        ...
        dispatch_once(&token, ^{
            // Create a dispatch queue for initializing data
            m_DQueue = dispatch_queue_create("com.apple.matrixmult.matrixrandvalues.main", DISPATCH_QUEUE_CONCURRENT);
