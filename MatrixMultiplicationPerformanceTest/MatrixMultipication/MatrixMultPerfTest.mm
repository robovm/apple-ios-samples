...
- (instancetype) init
{
    ...
    if(self)
    {
        ...
        dispatch_once(&token, ^{
            ...
            m_DQueue[0] = dispatch_queue_create("com.apple.matrixmult.perftest.main", DISPATCH_QUEUE_CONCURRENT);
            m_DQueue[1] = dispatch_queue_create("com.apple.matrixmult.perftest.rsrc", DISPATCH_QUEUE_SERIAL);
        });
...
- (void) _multiplyBlas
{
    ...
    dispatch_barrier_sync(m_DQueue[0], ^{
...
- (void) _avgLength
{
    ...
    // formed by the difference in the elements of matrices
