...
@private
    ...
    dispatch_queue_t  m_DQueue[2];          // Dispatch queues
}
...
- (instancetype) init
{
    ...
    if(self)
    {
        ...
        dispatch_once(&token, ^{
            ...
            m_DQueue[0] = dispatch_get_global_queue(QOS_CLASS_UTILITY, 0);
            m_DQueue[1] = dispatch_queue_create("com.apple.matrixmult.scheduler.inits", DISPATCH_QUEUE_CONCURRENT);
...
- (BOOL) _newList
{
    ...
    dispatch_barrier_sync(m_DQueue[1], ^{
        if(mpList != nullptr)
        {
            mnCount = count;
            mnSize  = size;
...
- (void) _addLogs:(const uint32_t)tid
{
    ...
    if(pTextDims)
    {
        NSString* pTextPerf = [NSString stringWithFormat:@">> [%d] Accelerate %f gflops/sec, Metal %f gflops/sec, Accelerate %f millisecs, Metal %f millisecs, Diff %e",
                               tidx,
                               [mpTest gflops:1],
                               [mpTest gflops:0],
                               [mpTest time:1],
        ...
        if(pTextPerf)
        {
            ...
            _logs[tid] = @{kMatrixLogTestID      : @(tidx),
                           kMatrixLogDimensions  : pTextDims,
                           kMatrixLogPerformance : pTextPerf};
...
{
    // Append a dictionary of key/value pairs to the array for test results
    _data[tid] = @{kMatrixMultTestID      : @(tid),
                   kMatrixARowCount       : @(mpTest.m),
                   kMatrixAColCount       : @(mpTest.n),
                   kMatrixBRowCount       : @(mpTest.n),
                   kMatrixBColCount       : @(mpTest.k),
                   kMatrixCPaddRowCount   : @(mpTest.M),
                   kMatrixCPaddColCount   : @(mpTest.K),
                   kMatrixMultGFlopsMetal : @([mpTest gflops:0]),
                   kMatrixMultGFlopsBlas  : @([mpTest gflops:1]),
                   kMatrixMultTimeMetal   : @([mpTest time:0]),
                   kMatrixMultTimeBlas    : @([mpTest time:1]),
                   kMatrixMultLength      : @(mpTest.length)};
               ...
               data:(float *)data
{
    ...
    dispatch_group_async(m_DGroup, m_DQueue[1], ^{
