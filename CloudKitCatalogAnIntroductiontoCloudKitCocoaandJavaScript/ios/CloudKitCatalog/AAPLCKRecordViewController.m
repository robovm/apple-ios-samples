...
@property (weak) IBOutlet UITextField *zoneTextField;
...
- (IBAction)saveRecord:(id)sender {
    ...
    if (self.zoneTextField.text.length > 0) {
        // Save the record to the specified zone.
        zone = [[CKRecordZone alloc] initWithZoneName:self.zoneTextField.text];
    }

    __weak __typeof(self) weakSelf = self;
    [self.cloudManager addRecordWithName:self.nameTextField.text location:saveLocation inZone:zone completionHandler:^(CKRecord *record) {
        ...
        if (record) {
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"CloudKit Catalog" message:@"Saved record." preferredStyle:UIAlertControllerStyleAlert];
